...
/**
 * Servlet implementation class regista
 */
@WebServlet("/regista")
public class regista extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			System.out.println("comenzamos");
				if(!db.VerificarId1(data.getInt("idac"))) {
					System.out.println("email correcto");
					boolean status = db.RegistrarA(data);
					if (status) {
						mensaje.put("status", 200).put("response", "Actividas fue creada");
